...
class AsyncGift(httpx.AsyncClient, NiosServiceMixin, NiosFileopMixin):
    ...
    @property
    def url(self) -> str:
        return f"https://{self.grid_mgr}/wapi/v{self.wapi_ver}" if self.grid_mgr and self.wapi_ver else ""

    async def connect(self, username: str = None, password: str = None, certificate: str = None):
        if not self.url:
    ...
    async def getone(self, wapi_object: str, params: Optional[dict] = None, **kwargs) -> str:
        res = await self.conn.get(f"{self.url}/{wapi_object}", params=params, **kwargs)
        res.raise_for_status()
        data = res.json()
        if len(data) != 1:
    ...
    async def post(self, wapi_object: str, json: Optional[dict] = None, **kwargs) -> httpx.Response:
