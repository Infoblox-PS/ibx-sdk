...
def test_stubzone_default_values():
    stub_zone = StubZone(fqdn="example.com", zone_format=ZoneFormatTypeEnum.FORWARD)
...
def test_stubzone_invalid_fqdn():
    with pytest.raises(ValidationError):
        StubZone(fqdn="", zone_format=ZoneFormatTypeEnum.FORWARD)


def test_stubzone_invalid_prefix():
    with pytest.raises(ValidationError):
        StubZone(fqdn="192.168.1.0", zone_format=ZoneFormatTypeEnum.IPV4, prefix=33)
...
def test_stubzone_serialize_stub_from():
    ...
    assert (
        stub_zone.serialize_list_fields(stub_zone.stub_from)
...
def test_stubzone_serialize_stub_members():
    ...
    assert (
        stub_zone.serialize_list_fields(stub_zone.stub_members)
